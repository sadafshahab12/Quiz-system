...
if (user_name.user) {
  ...
  let quiz5 = await inquirer.prompt({
    name: "question5",
    message:
    chalk.blueBright("What comes once in a minute, twice in a moment, but never in a thousand years?"),
    type: "list",
  ...
  console.log(
    chalk.redBright(`you percentage is ${(quiz_points / total_question) * 100}%`)
  );
} else {
